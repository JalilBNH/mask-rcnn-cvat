...
def handler(context, event):
    ...
    try:
        ...
        context.logger.info(f"Image received")
    ...
    for box, score, label, mask in zip(pred_boxes, scores, pred_classes, pred_masks):
        ...
        if contours:
            ...
            if contour.shape[0] >= 3:
                ...
                if contour.shape[0] < SAMPLE_NB:
                    points = contour.reshape(-1).tolist()
                else : 
                    points = []
                    sample_period = int(len(contour)/SAMPLE_NB)
                    for i, point in enumerate(contour):
                        if i % sample_period == 0:
                            points.append(int(point[0])), points.append(int(point[1]))
